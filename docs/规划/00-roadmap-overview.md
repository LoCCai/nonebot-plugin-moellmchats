...
| 16 | 平台 API 与插件接入链路优化计划书 | 16 | P0、P4、P1、P3、P2 已闭环；P5 观测字段已具备，仍须真实指标证明瓶颈 |
...
> 平台优化 P4（2026-09-07）：在分支整合最终基线 `ee99c7d…` 上实现 Bot 会话级协议能力探测缓存。成功结果固定 300 秒 TTL、256 会话 LRU 并合并同会话并发 miss；失败不缓存，等待者取消不取消共享探测，重连及可见实现/版本变化隔离。二阶段确认仍强制刷新且刷新失败清除旧成功记录。实现提交 `ecfb32cac1da0fb616a344744949a86bb3ce5de7` 的 push run [`34079965087`](https://github.com/LoCCai/nonebot-plugin-moellmchats/actions/runs/34079965087) 为 12/12 success、唯一 `release-gate` 成功；完整本地与远端证据见 [16 号计划书](./16-platform-api-and-catalog-optimization-plan.md)。未操作七七或生产。

> 平台优化独立审计复核（2026-09-07，实施者之外的第二方复核）：对 P4/P1/P3/P2/P5 五个实现提交（`ecfb32c`/`8521ad1`/`915fc34`/`d508101`/`1d18b0e`）逐 diff 审阅完成。总体结论：**全部按 16 号计划书设计落地，P1 的 plain_text 正确性陷阱已正确闭环**——`business_conflict_digest` 作为显式键因子、capture 与渲染消费同一份摘除集合、"键相同 ⇒ 产物一致"成立；digest 剔除 message_id 无遗漏路径；测试直接覆盖"同消息共享键 + 业务冲突隔离"场景。P4 的 weakref 会话绑定/单飞/TTL、P3 的二态描述视图、P2 的按天抽验防回退闸门均符合设计。**新发现一项低优先隐患 R-1**：四处 record 构建器重读全局业务冲突状态而非复用 context 集合（当前同步构建下正确；延迟构建场景存在键产物错位隐患 + 重复计算），已记入 [16 号审计节](./16-platform-api-and-catalog-optimization-plan.md)与 [15 号清单](./15-pending-issues-backlog.md)。审计结论：P0～P4 判定完成，P5 维持"仅观测、待真实指标"状态。
...
> I-05 本地门禁（2026-08-24）：实现提交 `eba88c54faf63f9693f61615a54151941c30a23f` 新增无模块级实例的 `RuntimeResourceSettings / RuntimeResourceBuilder / RuntimeGenerationResources / RuntimeResourceManager`，按 generation 组合 snapshot、完整 PostgreSQL Repository provider、Memory/惰性 Redis cache、Usage/Audit queue、Full Metrics、structured logger、API handlers 与可选 trusted runner。启动按声明顺序、关闭/部分启动失败按逆序，取消会等待收尾后重抛；reload 先启动并发布新代，再等待旧代 lease 排空，关闭失败代可重试。lease 绑定请求所见 generation，嵌套调用保持旧代，逃逸子任务在父 lease 结束后 fail closed，lease 内 reload/close 立即拒绝以避免自死锁；未确认 queue 记录阻止退休且不丢弃。四版本定向各 `34 passed`、相关联合 `1101 passed`、四版本及 Python 3.10 最低依赖全量各 `2738 passed, 1 skipped`，mandatory root Sandbox `41 passed, 0 skipped`；Ruff/Pyright、104 成员 fresh 制品/同哈希重建及 Python 3.10/3.12 × wheel/sdist 四组包外 11 表/8 revision/离线 DDL/resource start/reload/close/零真实 I/O smoke 均通过。wheel/sdist SHA256 为 `88635a33b1ab42c12067e4fea51e81f69c56d2d58dc3e666a4c7c220fd8d0243` / `ce8cfd0827925ab0282c28781efba4ea38a6738121d1df6aacc3744d69f870c4`。精确 HEAD 双 run 待完成，I-06 继续锁定；未接 `__init__.py`/真实聊天入口，未读取连接信息、未连接真实 PostgreSQL/Redis/模型、未迁移、未合并、发布、部署或重启。

> I-05 远端闭环（2026-08-24）：本地证据 HEAD `fe4e4e3d78e0fe8ef6917d380529062465c7f7c6` 的 push run `32694202902` / PR run `32694205818` 均精确命中该 SHA，各 11/11 success、`non_success=[]`，并各恰好一个 `completed/success release-gate`。本地、origin、`ls-remote` 与 PR head 四方一致，PR #2 为 `OPEN / MERGEABLE / CLEAN`。I-05 门禁已关闭，I-06 前置依赖已解除；未合并、promotion、发布、部署或重启，未运行 migration，未连接真实 PostgreSQL/Redis/模型。

> I-06 本地门禁（2026-08-24）：I-05 最终文档 HEAD `1dc7dd4fb3fdb29b37bd2be4a4f904103e19108d` 的 push `32694556611` / PR `32694558961` 已各 11/11 success、`non_success=[]`、各唯一 `release-gate` 成功。在此前提下，实现提交 `a0dba24eab16da2deeecacd2981848a124467a59` 新增 generation-local Agent context coordinator/host，并接入 NoneBot startup/shutdown、真实 `handle_llm`、MessagesHandler、模型与工具生命周期：单一 Deadline 驱动 AgentRun/Step/ToolCall，Memory 默认零后端 I/O；显式 PostgreSQL 只用短事务，commit unknown/取消 unknown 不重放，durable message commit 后才 invalidate cache，cache 不可信即整代旁路；summary/LTM 失败降级但取消传播，usage 非关键失败降级，关键 Agent/audit 写失败拒绝伪造成功。四版本定向各 `26 passed`、联合 `1024 passed`，四版本与 Python 3.10 最低依赖全量各 `2786 passed, 1 skipped`，mandatory root Sandbox `41 passed, 0 skipped`；Ruff/Pyright、105 成员 fresh wheel/sdist/Twine、sdist 重建字节一致及四组包外 11 表/8 revision/Memory Agent 生命周期/零真实 I/O smoke 均通过。wheel/sdist SHA256 为 `81ac1d112b8f5f74c6d1bd6c7bbe52920727732bda565cda5d42141606168a85` / `11b1b556a705ab7022c61fa6a853251a0cf41bdd808ff8709a006769d4df7369`。精确 HEAD 双 run 待完成，I-07 继续锁定；未读取连接信息或凭据，未连接真实服务、未运行 migration、未合并、发布、部署或重启。

> I-06 远端闭环（2026-08-24）：本地证据 HEAD `fe3b48f212de1e79bdcad7c1f48c456bc3f317a8` 的 push `32703751436` / PR `32703756205` 均精确命中该 SHA、各 11/11 success、`non_success=[]`、各唯一 `completed/success release-gate`；本地、origin、`ls-remote` 与 PR head 四方一致，PR #2 为 `OPEN / MERGEABLE / CLEAN`。I-06 已完成，I-07 前置依赖解除；进入 I-07 前仍须本最终闭环文档 HEAD 通过同样双门禁。未合并、发布、部署、迁移或连接真实服务。

> I-06 最终闭环（2026-08-24）：最终文档 HEAD `caf6e2c0f7d603835964042d7fae124e7c83a12f` 的 push `32704551636` / PR `32704555524` 均精确命中该 SHA、各 11/11 success、`non_success=[]`、各唯一 `completed/success release-gate`；本地、origin、`ls-remote` 与 PR head 四方一致，PR #2 为 `OPEN / MERGEABLE / CLEAN`。I-07 实现依赖已完全关闭；未合并、发布、部署、迁移或连接真实服务。

> I-07 本地门禁（2026-08-24）：实现提交 `37abc1e6db908c3e826ee7548900cd336b669f9c` 在真实 `_execute_tools()` 路径接入 G-09/G-10；只有同一 generation 的 `ToolGraph` 与 `TrustedRunnerPool` 双重显式配置，且全批调用均为 provider-authoritative、trust 允许、强类型 `READ_ONLY`、无 capability/确认要求、allowlist 命中、依赖闭包完整并有显式 `parallel_with` 时才进入并行。mutating、冲突、确认、重复、非 allowlist、缺依赖或 generation 漂移均回退原串行/拒绝语义。并行调用共享单一 Deadline，首错取消并 drain，按原请求顺序回填；trace 用请求局部锁串行持久化以保证 step index 唯一，关键写失败 fail closed 且不重放未知结果。四版本定向各 `68 passed`、联合回归 `471 passed`，四版本及 Python 3.10 最低依赖全量各 `2799 passed, 1 skipped`，mandatory root Sandbox `41 passed, 0 skipped`；Ruff 0.16.2 全仓通过，Pyright 1.1.407 目标零诊断且全仓相对 I-06 `added=0 / removed=43`。105 成员 fresh wheel/sdist、Twine、sdist 重建字节一致及 Python 3.10/3.12 × wheel/sdist 四组包外 11 表/8 revision/Memory Agent/真实并发度 2/原序回填/trace 唯一/零真实 I/O smoke 全绿。wheel/sdist SHA256 为 `3318fde6277d06c56baf4bc29bb7aa03bdf056680df2f47c207342c935ea46ff` / `8ccba97c4c3a82e49d49035397e476b57069fb36265f81e505311edf25a00144`。精确 HEAD 双 run 待完成，I-08 继续锁定；未读取凭据、未连接真实 PostgreSQL/Redis/模型、未运行 migration、未合并、发布、部署或重启，用户的 `uv.lock` 未修改、未暂存、未提交。

> I-07 远端闭环（2026-08-24）：本地证据 HEAD `f00476245f96c3d50a98399452febb8fc21aa17b` 的 push `32712268122` / PR `32712272403` 均精确命中该 SHA、各 11/11 success、`non_success=[]`、各唯一 `completed/success release-gate`；本地、origin、`ls-remote` 与 PR head 四方一致，PR #2 为 `OPEN / MERGEABLE / CLEAN`。I-07 已完成，I-08 前置依赖解除；进入 I-08 前仍须本最终闭环文档 HEAD 通过同样双门禁。未合并、发布、部署、迁移或连接真实服务。

> I-07 最终闭环（2026-08-24）：最终文档 HEAD `9fd1871a6e039a10c1f374f25b8db113016aa3ef` 的 push `32713316379` / PR `32713320021` 均精确命中该 SHA、各 11/11 success、`non_success=[]`、各唯一 `completed/success release-gate`；本地、origin 与 PR head 一致，PR #2 为 `OPEN / MERGEABLE / CLEAN`。I-08 实现依赖据此完全关闭；未触发重跑或新 CI，未合并、发布、部署、迁移或连接真实服务。

> I-08 本地门禁（2026-08-24）：实现提交 `abc275721b67165224309d79e4406e95012f2975` 基于已闭环的 `9fd1871a6e039a10c1f374f25b8db113016aa3ef`，完成受信 generation-bound model routing、平台 mounts/metrics/logging、Usage/Audit spool/worker、Redis 组合故障策略，以及三类 cache consumer。Tool Catalog Cache 接入 `Categorize`，Tool Schema Cache 接入异步预备与同步 payload 物化，Classification Cache 接入成功模型分类；generation/config/key 漂移与 cache backend 故障 fail closed，timeout/parse/content-blocked 结果不发布。三类 consumer 文件 `12 passed`，扩展联合回归 `1094 passed`；Python 3.10/3.11/3.12/3.13 与 Python 3.10 最低依赖全量均为 `2874 passed, 1 skipped`，mandatory root Sandbox `41 passed, 0 skipped`，Ruff、diff/目标 format 与目标 Pyright 零诊断。fresh wheel/sdist 各 111 成员，SHA256 为 `1b7503c8d815d86c1f5f865290e41298a1b8e41b2a42e8f262c9f8376affa3de` / `09adc954287ddc5953fc7afd40cc961beef9f35253b32ee943ac7f49bf9563ea`，Twine、sdist 重建字节一致；Python 3.10/3.12 × wheel/sdist 四组均从 site-packages 加载，原有 11 表/8 revision/API/spool/routing 零真实 I/O smoke 与新增 cache consumer `12 passed` 全绿。制品目录 `/tmp/moellm-i08-cache-artifacts.0kYH3Y`。未读取凭据、未连接真实 PostgreSQL/Redis/模型、未运行 migration，未合并、发布、部署或重启，用户未跟踪的 `uv.lock` 未修改、未暂存、未提交。

> I-08 远端闭环（2026-08-24）：本地证据 HEAD `d77986d7e724758f24ad53fac9806e7482938ef4` 的 push `32742181099` / PR `32742192391` 均精确命中该 SHA、各 11/11 success、`non_success=[]`、各唯一 `completed/success release-gate`；本地、origin、`ls-remote` 与 PR head 四方一致，PR #2 为 `OPEN / MERGEABLE / CLEAN`。I-08 本地证据门禁已关闭，I-09 依赖解除；进入 I-09 前仍须本最终闭环文档 HEAD 通过同样双门禁。未合并、发布、部署、迁移或连接真实服务。

> I-08 最终闭环（2026-08-24）：最终文档 HEAD `5f711ffe25b5bd29ccd65278fae30e6d1b4777b9` 的 push `32742896973` / PR `32742899876` 均精确命中该 SHA、各 11/11 success、`non_success=[]`、各唯一 `completed/success release-gate`；本地、origin、`ls-remote` 与 PR head 四方一致，PR #2 为 `OPEN / MERGEABLE / CLEAN`。I-09 实施依赖据此完全关闭；未合并、发布、部署、迁移或连接真实服务。

> I-09 本地最终矩阵（2026-08-24）：隔离根目录 `/tmp/moellm-i09-matrix.wSgBFe` 中，Python 3.10/3.11/3.12/3.13 普通全量与 Python 3.10 最低依赖全量各 `2874 passed, 1 skipped`，最低依赖为 SQLAlchemy 2.0.0、Alembic 1.13.0、asyncpg 0.30.0、Redis 5.2.0、FakeRedis 2.31.0；mandatory root Sandbox 为 `41 passed`，JUnit 为 `tests=41 / failures=0 / errors=0 / skipped=0`。Ruff 0.16.2 全仓通过，18 个新增/consumer 目标文件 format 通过，Pyright 1.1.407 目标模块/测试为 `0 errors, 0 warnings`；扩大到既有大文件仍会复现旧 format/Pyright 基线，因此不声明全仓 format/Pyright 零诊断。fresh wheel/sdist 各 111 成员，SHA256 为 `1b7503c8d815d86c1f5f865290e41298a1b8e41b2a42e8f262c9f8376affa3de` / `09adc954287ddc5953fc7afd40cc961beef9f35253b32ee943ac7f49bf9563ea`，Twine 通过且 sdist 重建 wheel 字节一致；Python 3.10/3.12 × wheel/sdist 四组均从目标 site-packages 加载，验证 11 表、8 revision、API/spool/routing 生命周期、spool `0700` 与每组 cache consumer `12 passed`，engine/asyncpg/Redis/socket I/O 均为 0。Plan 2 / Plan 3 开发态验收完成；本地证据 HEAD 的远端双门禁已关闭，生产观察与 D-09 仍不在本轮完成口径内。
