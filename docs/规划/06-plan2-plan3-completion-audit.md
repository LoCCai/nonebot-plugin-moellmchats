...
- 上述 H-08 证据只证明已拆分 primitive 的质量，不等于 Plan 2 / Plan 3 的最终运行态验收。此后 I-06 已在开发仓库真实聊天路径创建 Agent 领域记录并消费 history/summary/LTM/usage/audit 能力，本地证据 HEAD 的精确双 run 已关闭；I-07 仅等待本最终闭环文档 HEAD 自身双 run，其余能力继续按 I-07～I-09 收口。
...
| AgentRun / AgentStep / ToolCall / Deadline | I-06 已让真实 `handle_llm` 租用 generation resource，以单一 Deadline 创建并持久化 AgentRun、模型/工具 Step 与完整 ToolCall 状态轨迹，本地证据 HEAD 双 run 已关闭 | I-07 等待本最终闭环文档 HEAD 自身双 run，且尚未让并行工具复用该 deadline/trace |
...
| Agent persistence | I-06 已以 caller-owned `AsyncSession` 短事务接入 User/Conversation/Message/Summary/AgentRun/AgentStep/ToolCall/Usage/Audit Repository，commit unknown 不重放，本地证据 HEAD 双 run 已关闭 | 未运行生产 migration，I-08 spool 尚未实现 |
| History / Summary / Long-Term Memory | I-06 已接 committed MessagesHandler、hot-cache trust、summary CAS watermark 与显式 LTM untrusted prompt，定义失败降级/取消传播，本地证据 HEAD 双 run 已关闭 | 真实 PostgreSQL/Redis/模型生产观察不在本轮范围 |
...
- AgentRun/AgentStep/ToolCall、单一 Deadline、history/summary/LTM 与 structured audit 路径（I-06 本地证据双 run 已关闭）
...
- capability routing、parallel executor 与平台 observability/API 的真实消费
...
- 本地证据 HEAD `fe3b48f212de1e79bdcad7c1f48c456bc3f317a8` 的 push `32703751436` / PR `32703756205` 各 11/11 success、`non_success=[]`、唯一 `release-gate` 成功，四方 HEAD 一致且 PR #2 为 `OPEN / MERGEABLE / CLEAN`。I-06 已完成，I-07 依赖解除；进入实现前仍须本最终闭环文档 HEAD 自身双 run。未迁移、未连接真实服务、未合并、发布、部署或重启。
...
当前精确恢复点：规划审计基线与 I-01～I-06 本地证据 HEAD 的 push/PR 双 `release-gate` 均已关闭；I-06 最终闭环文档正在形成，下一步让该文档 HEAD 自身通过精确双 run。完成前不开始 I-07；仍不运行在线 migration、不连接真实服务、不操作生产。
