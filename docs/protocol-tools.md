...
| --- | --- | ---: | --- |
| OneBot v11 通用实现 | Adapter 是 OneBot v11；会话缓存未命中时以独立 3 秒上限调用一次 `get_version_info` | 38 个 v11 公开动作 | 只保留人工策略允许、当前用户/场景可执行的 v11 动作 |
...
| OneBot v12 | Adapter 是 OneBot v12；会话缓存未命中时以独立 3 秒上限调用一次 `get_supported_actions` | 31 个 v12 标准动作 | 包内清单与 Bot 返回动作的交集 |
...
成功的实现/动作探测会在当前 Bot 会话内缓存 300 秒，最多保留 256 个会话；同一会话的并发 miss 只发起一次只读 API，某个等待请求被取消不会取消其他等待者共用的探测。缓存只保存实现、版本和动作集合，不保存用户、消息、群或正文；每条消息仍重新生成自己的不可变权限/场景快照。探测超时、异常或非法响应不会缓存，下一请求会重新尝试。新 Bot 会话、Adapter/协议/Bot ID 或可见实现/版本提示变化会自然使用新键，其他无法提前观察的服务端版本变化最多受 300 秒 TTL 限制。

二阶段确认是例外：确认执行前会丢弃该会话的普通命中并强制重新探测；刷新失败同时移除旧成功记录，动作保持拒绝。这样，日常聊天避免每消息探测，危险副作用仍遵守“执行前重新校验全部条件”。
...
目录和 Schema 缓存键包含协议、实现、实现版本、支持动作摘要、Adapter、Bot、调用者、SUPERUSER 身份、场景、群/频道、当前消息与回复是否存在、runtime generation，以及本次业务菜单需要抑制的协议工具集合摘要。具体 `message_id` / `reply_message_id` 仍保留在不可变请求快照中用于执行，但不进入缓存键；所以同一作用域的连续消息可以复用目录，而不同 Bot、协议、权限、场景、消息/回复可用性或业务冲突集合仍不会交叉复用。冲突集合在查缓存前一次性冻结，键中只放 canonical SHA-256，不放用户原文或工具名。这里的目录/Schema 缓存与前述只保存会话能力的 300 秒探测缓存不是同一层。
