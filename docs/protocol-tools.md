...
目录和 Schema 缓存键包含协议、实现、实现版本、支持动作摘要、Adapter、Bot、调用者、SUPERUSER 身份、场景、群/频道、当前消息、回复消息和 runtime generation。因此不同 Bot、协议、消息或权限不会共用协议目录。这里的目录/Schema 缓存与前述只保存会话能力的 300 秒探测缓存不是同一层。
