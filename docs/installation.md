...
- 当前推荐进入隔离测试的 0.26.4 精确 Git 提交是 `2b87cdf410b3c77792b5d8c9d37ab11b379d72c8`。
- 该提交包含全量 OneBot/NapCat 协议工具、固定冷却管理入口、0.26.2 的业务路由/真实状态、0.26.3 的连续取消/single-flight/安全 HTTP，以及 0.26.4 的分类超时、400 正文边界、参数级重复限次和固定工具进度指令。
...
- 包内版本号是 `0.26.4`，但这不表示 PyPI 已发布该版本。`pip install nonebot-plugin-moellmchats` 得到什么仍必须以实际索引为准。
- 这些证据说明“开发制品可以进入隔离测试”，不等于它已经在七七或其他生产 Bot 中部署验证。

安装 0.26.4 时必须固定到完整的 `2b87cdf…` SHA。只有明确回退到 0.26.3 时才使用 `86ee2a6…`，回退到 0.26.2 时才使用 `e340fb7…`，回退到 0.26.1 时才使用 `5d7f795…`，回退到 0.25.0 时才使用 `79d2268…`。不要改用移动分支，也不要把本地脏工作树或未绑定提交的临时制品当作可恢复安装来源。
