...
默认情况下，分类模型会在每次对话时决定是否注入某个工具的 schema。如果希望某个 NoneBot 插件、文件/生成工具或 MCP 工具**每次都强制注入**（不经过分类模型判断），可将其添加到常驻列表。
...
也可直接在 `model_config.json` 的 `resident_plugins` 数组中手动添加工具标识。

---

## 插件黑名单（tool_blacklist）

某些系统级或危险工具不应被 LLM 调用，可加入黑名单：
