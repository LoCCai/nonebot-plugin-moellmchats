...
| `/设置工具进度` / `!设置调用进度` / `！设置工具提示` `<开关>` | 私聊/群聊 | `0`/`1`/`开`/`关` | 工具进度总开关；开启后每个获准调用各发送一条可信固定提示，关闭不影响实际调用、确认、结果、最终回复和日志 |
| `/设置工具自然话术` / `设置工具话术` / `设置调用话术` `<开关>` | 私聊/群聊 | `0`/`1`/`开`/`关` | 是否把当前工具决策响应中的一句脱敏自然话术合并进固定提示；默认关闭、不增加模型请求，总开关关闭时不生效 |
| `设置私聊 <开关>`                        | 私聊/群聊 | `0`/`1`/`开`/`关` | 是否允许超级管理员私聊 Bot           |
| `/设置LLM冷却` / `!设置LLMCD` / `！设置对话冷却` `<秒数>` | 私聊/群聊 | `0`～`86400` | 全文固定匹配并直接调整每用户对话冷却；前缀也可省略，`0` 表示关闭，不经过 LLM、冷却或生成工具 |

工具进度、工具自然话术与冷却命令都是独立的优先级 0 管理入口，只接受整条固定指令，且仅 `SUPERUSER` 可执行。它们有意不依赖 NoneBot 标准命令前缀预处理，避免其他插件的命令 Trie 或现场前缀状态使管理入口失效；普通群管理员不会因此获得修改全局配置的权限。
