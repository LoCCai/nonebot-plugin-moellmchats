...
| `/设置工具进度` / `!设置调用进度` / `！设置工具提示` `<开关>` | 私聊/群聊 | `0`/`1`/`开`/`关` | 是否向当前会话发送工具调用前的过渡话术及“正在搜索/调用/执行”提示；不影响实际调用、确认、结果、最终回复和日志 |
...
工具进度与冷却命令都是独立的优先级 0 管理入口，只接受整条固定指令，且仅 `SUPERUSER` 可执行。它们有意不依赖 NoneBot 标准命令前缀预处理，避免其他插件的命令 Trie 或现场前缀状态使管理入口失效；普通群管理员不会因此获得修改全局配置的权限。
