...
def test_packaged_migration_graph_is_linear_and_returns_fresh_diagnostics() -> None:
    ...
    assert graph == DatabaseMigrationGraph(
        revisions=("0001_users_conversations", "0002_agent_runtime", "0003_agent_steps"),
        bases=("0001_users_conversations",),
        heads=("0003_agent_steps",),
    )
...
def test_revision_template_generates_valid_python_in_temporary_copy(tmp_path: Path) -> None:
    ...
    ast.parse(source)
    assert revision.revision == "0004_example"
    assert revision.down_revision == "0003_agent_steps"
    assert "down_revision: str | Sequence[str] | None = '0003_agent_steps'" in source
