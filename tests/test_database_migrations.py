...
def test_packaged_migration_graph_is_linear_and_returns_fresh_diagnostics() -> None:
    ...
    assert graph == DatabaseMigrationGraph(
        ...
        heads=("0004_tool_calls",),
    )
    assert graph.revision_count == 4
...
def test_revision_template_generates_valid_python_in_temporary_copy(tmp_path: Path) -> None:
    ...
    ast.parse(source)
    assert revision.revision == "0005_example"
    assert revision.down_revision == "0004_tool_calls"
    assert "down_revision: str | Sequence[str] | None = '0004_tool_calls'" in source
