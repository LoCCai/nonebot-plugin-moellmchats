...
def test_packaged_migration_layout_is_complete() -> None:
    ...
    assert (location / "versions" / "0003_agent_steps.py").is_file()
    assert (location / "versions" / "0004_tool_calls.py").is_file()
    assert (location / "versions" / "0005_tool_bundle_metadata.py").is_file()
    assert (location / "versions" / "0006_audit_events.py").is_file()
    assert (location / "versions" / "0007_model_usage.py").is_file()
    assert (location / "versions" / "0008_session_summaries.py").is_file()
...
def test_packaged_migration_graph_is_linear_and_returns_fresh_diagnostics() -> None:
    ...
    assert graph == DatabaseMigrationGraph(
        revisions=(
            ...
            "0006_audit_events",
            "0007_model_usage",
            "0008_session_summaries",
        ),
        bases=("0001_users_conversations",),
        heads=("0008_session_summaries",),
    )
    assert graph.revision_count == 8
...
def test_revision_template_generates_valid_python_in_temporary_copy(tmp_path: Path) -> None:
    ...
    ast.parse(source)
    assert revision.revision == "0009_example"
    assert revision.down_revision == "0008_session_summaries"
    assert "down_revision: str | Sequence[str] | None = '0008_session_summaries'" in source
...
def test_offline_upgrade_renders_without_connecting(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "CREATE TABLE agent_steps" in rendered
    assert "CREATE TABLE tool_calls" in rendered
    assert "CREATE TABLE tool_bundles" in rendered
    assert "CREATE TABLE tool_bundle_versions" in rendered
    assert "CREATE TABLE audit_events" in rendered
    assert "CREATE TABLE model_usage" in rendered
    assert "CREATE TABLE session_summaries" in rendered
    ...
    assert "arguments_json JSONB" in rendered
    assert "manifest_json JSONB" in rendered
    assert "risks_json JSONB" in rendered
    assert "capabilities_json JSONB" in rendered
    assert "metadata_json JSONB" in rendered
    assert "reasoning_tokens BIGINT NOT NULL" in rendered
    assert "cached_tokens BIGINT NOT NULL" in rendered
    assert "source_digest VARCHAR(64) NOT NULL" in rendered
    assert "CONSTRAINT uq_messages_conversation_id_id UNIQUE (conversation_id, id)" in rendered
    assert "CONSTRAINT uq_session_summaries_conversation_generation UNIQUE (conversation_id, generation)" in rendered
    assert "CONSTRAINT fk_session_summaries_previous_summary" in rendered
    assert "CONSTRAINT uq_agent_steps_run_id_id UNIQUE (run_id, id)" in rendered
    assert "CONSTRAINT uq_tool_calls_run_id_id UNIQUE (run_id, id)" in rendered
    assert "CONSTRAINT uq_tool_bundle_versions_bundle_id_digest UNIQUE (bundle_id, digest)" in rendered
    assert "ADD CONSTRAINT fk_tool_bundles_active_version" in rendered
    ...
    assert rendered.index("CREATE TABLE agent_runs") < rendered.index("CREATE TABLE agent_steps")
    assert rendered.index("CREATE TABLE agent_steps") < rendered.index("CREATE TABLE tool_calls")
    assert rendered.index("CREATE TABLE tool_calls") < rendered.index("CREATE TABLE tool_bundles")
    assert rendered.index("CREATE TABLE tool_bundles") < rendered.index("CREATE TABLE tool_bundle_versions")
    assert rendered.index("CREATE TABLE tool_bundle_versions") < rendered.index("CREATE TABLE audit_events")
    assert rendered.index("CREATE TABLE audit_events") < rendered.index("CREATE TABLE model_usage")
    assert rendered.index("CREATE TABLE model_usage") < rendered.index("CREATE TABLE session_summaries")


def test_session_summaries_downgrade_only_removes_summary_support_without_connecting(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def unexpected_connection(*_args: Any, **_kwargs: Any) -> None:
        pytest.fail("offline migration attempted to create a database engine")
    ...
    assert "DROP TABLE session_summaries" in rendered
    assert "DROP CONSTRAINT uq_messages_conversation_id_id" in rendered
    assert "DROP TABLE model_usage" not in rendered
    assert "DROP TABLE messages" not in rendered


def test_model_usage_downgrade_only_removes_usage_table_without_connecting(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def unexpected_connection(*_args: Any, **_kwargs: Any) -> None:
        pytest.fail("offline migration attempted to create a database engine")
    ...
    rendered = output.getvalue()
    assert "DROP TABLE model_usage" in rendered
    assert "DROP TABLE audit_events" not in rendered
    assert "DROP TABLE agent_runs" not in rendered


def test_audit_events_downgrade_only_removes_audit_support_without_connecting(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def unexpected_connection(*_args: Any, **_kwargs: Any) -> None:
        pytest.fail("offline migration attempted to create a database engine")
    ...
    rendered = output.getvalue()
    assert "DROP TABLE audit_events" in rendered
    assert "DROP CONSTRAINT uq_tool_calls_run_id_id" in rendered
    assert "DROP TABLE tool_bundle_versions" not in rendered
    assert "DROP TABLE tool_bundles" not in rendered
    assert "DROP TABLE tool_calls" not in rendered


def test_tool_bundle_metadata_downgrade_only_removes_bundle_tables_without_connecting(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def unexpected_connection(*_args: Any, **_kwargs: Any) -> None:
        pytest.fail("offline migration attempted to create a database engine")
    ...
    rendered = output.getvalue()
    assert "DROP CONSTRAINT fk_tool_bundles_active_version" in rendered
    assert rendered.index("DROP TABLE tool_bundle_versions") < rendered.index("DROP TABLE tool_bundles")
    assert "DROP TABLE tool_calls" not in rendered
    assert "DROP TABLE agent_steps" not in rendered


def test_tool_calls_downgrade_only_removes_tool_calls_support_without_connecting(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def unexpected_connection(*_args: Any, **_kwargs: Any) -> None:
        pytest.fail("offline migration attempted to create a database engine")
    ...
    rendered = output.getvalue()
    assert "DROP TABLE tool_calls" in rendered
    assert "DROP CONSTRAINT uq_agent_steps_run_id_id" in rendered
    assert "DROP TABLE agent_steps" not in rendered
    assert "DROP TABLE agent_runs" not in rendered


def test_agent_steps_downgrade_only_removes_agent_steps_without_connecting(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def unexpected_connection(*_args: Any, **_kwargs: Any) -> None:
        pytest.fail("offline migration attempted to create a database engine")
    ...
    rendered = output.getvalue()
    assert "DROP TABLE agent_steps" in rendered
    assert "DROP TABLE agent_runs" not in rendered
    assert "DROP TABLE messages" not in rendered
