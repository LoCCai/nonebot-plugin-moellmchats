...
def test_custom_http_request_promotes_writes_but_not_gets(
    ...
    source.write_text(
        ...
        "        'https://example.invalid', method='POST', body='value'\n"
        "    )\n"
        "async def inspect():\n"
        "    return await safe_request(\n"
        "        'https://example.invalid', method='GET'\n"
        "    )\n"
        "async def inspect_unbound():\n"
        "    fetch = safe_request\n"
        "    return await fetch('https://example.invalid', method='HEAD')\n"
        "async def send_unbound():\n"
        "    return await safe_request(\n"
        "        'https://example.invalid', method='DELETE'\n"
        "    )\n"
        "TOOLS_REGISTRY = [\n"
        "  {\n"
...
def test_custom_capability_is_checked_for_each_handler_call_graph(
    ...
    template = (
        "async def _fetch():\n"
        "    return await safe_request('https://example.invalid')\n"
        "async def online():\n"
        "    return await _fetch()\n"
        "async def local():\n"
        "    return 'local'\n"
