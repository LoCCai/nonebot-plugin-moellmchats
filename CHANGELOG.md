...
- NoneBot 兼容调度的既有完成日志增加固定低基数 `protocol`、`mode` 与 `_build_event()` 的 `event_build_us`，配合累计 `dispatch_modes` 和整步 `duration_ms` 判断 v12 full-bus 合成事件是否真是瓶颈；不新增日志条数，也不记录正文、命令、身份、URL、路径或 API 参数。P5 字段白名单仍须真实运行指标证明后才实施。
